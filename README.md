...
Regenerated by `make train-ci`. Synthetic morphology tags only — not clinical performance. A committed copy of the logistic plot lives at [`docs/confusion_logistic.png`](docs/confusion_logistic.png); `reports/confusion_*.png` from a fresh train are gitignored.

## Demo / Results

Synthetic CI metrics from a real `make train-ci` run (2026-09-13). **n_per_class=24**, length=256, split 66/15/15, leakage_ok=true. Simulator scores only — not clinical performance.
